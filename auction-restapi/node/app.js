...
const {
   errorResponse,
} = require('./app/object-builders/client-response-builder')
const {
   clientErrorHandler,
   errorHandler,
} = require('./app/helpers/errorhandlers')
...
// error handling
app.use((req, res) =>
   res.status(404).send(errorResponse.format('Router not found'))
)
...
const server = http.createServer(app).listen(port, async function () {
   await Promise.all([helper.enrollAdmin('org1')]).then(
      result => {
