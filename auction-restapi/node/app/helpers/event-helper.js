...
eventHelper.registerEvent = async function (sendAll) {
   ...
   if (!adminIdentity) {
      logger.debug(
         `An identity for the user ${adminUserId} does not exist in the wallet`
      )
      return
   }
   ...
   try {
      ...
      listener = async event => {
         if (event.eventName == 'TRANSFER_ITEM') {
            ...
            // transfer the asset
            nftController.transferNftEvent(
               asset,
               adminUserId,
               config.auctionHouseOrgName
            )
         }
      }
      // start the client side event service and register the listener
      logger.debug(
         `--> Start contract event stream to peer in ${config.auctionHouseOrgName}`
      )
         ...
         for (const transEvent of transEvents) {
            ...
            if (transEvent.transactionData) {
               ...
               for (let i in event.blockData.data.data) {
                  try {
                     tx = {
                        tx_id: event.blockData.data.data[i].payload.header
                           .channel_header.tx_id,
                        timestamp: moment(
                           Date.parse(
                              event.blockData.data.data[i].payload.header
                     ...
                     logger.warn(
                        'error in removing buffers - this does not matter',
                        e
                     )
                  }
