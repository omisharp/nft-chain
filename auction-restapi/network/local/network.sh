...
function networkUp() {
  checkPrereqs
  # remove wallet identities if they already exist
  rm -Rf ./identities
...
function createChannel() {
  ...
  scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE $CHANNEL_PROFILE
}
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    if [ "$OS_ARCH" == "darwin-amd64" ]; then
      ...
      ## remove fabric ca artifacts
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/org1'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/org2'
...
CLI_DELAY=3
# channel name defaults to "defaultchannel"
CHANNEL_NAME="defaultchannel"
# channel profile defaults to "defaultchannel"
CHANNEL_PROFILE="DefaultChannel"
