...
module.exports.enrollAdmin = async orgName => {
   try {
      ...
      let walletPath = path.join(
         __dirname,
         `../../network/${process.env.NODE_ENV}/identities/wallet`
      )
...
module.exports.registerAndEnrollUser = async (
   username,
   secret,
   userOrg,
   isJson
   ...
   try {
      ...
      if (
         error.message.includes('Calling register endpoint failed') ||
...
module.exports.userSignature = async function (username, msg) {
   try {
      ...
      logger.error(
         'Failed to sign data with user signature: ' + err.stack
            ? err.stack
            : err
      )
      throw new Error(
         'Failed to sign data with user signature: ' + err.toString()
      )
   }
}
