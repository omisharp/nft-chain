      ...
      <div className="col-md-6 col-lg-4">
        <div className={"card artwork-card mb-4 " + (isAuctionClosed ? 'artwork-card-disabled' : '')}>
          ...
          <div className="card-body">
            ...
            {isAuction && <p>Giá mua ngay: <strong>${parseInt(buyItNowPrice, 10).toLocaleString()}</strong></p>}
            <div className="d-flex justify-content-between align-items-center">
              {isAuction && !isAuctionClosed && <button onClick={() => this.props.handleClick(id, nftId)} type="button" className={"btn btn-sm btn-outline-" + (isAuction ? 'danger' : 'primary')} data-toggle="modal" data-target=".art-detail-modal">Trả giá</button>}
              ...
              {isAuction && isAuctionClosed && <span className="badge badge-danger">Đã đóng</span>}
            </div>
          </div>
        </div>
      </div>
        ...
        {nfts.map((nft, idx) => {
          return <div id={"nft-" + idx} key={idx} className={"card artwork-card mb-4 " + (isAuctionClosed ? 'artwork-card-disabled' : '')}>
            ...
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center">
                <button onClick={() => this.props.handleClick(id, nft.nftId, idx)} type="button" className="btn btn-sm btn-outline-primary" data-toggle="modal" data-target=".art-detail-modal">Hiển thị chi tiết</button>
                <div className="btn-group">
                  {nft.nftStatus === 'INITIAL' && <button onClick={() => this.props.handleClick(id, nft.nftId, idx)} type="button" className="btn btn-sm btn-outline-success" data-toggle="modal" data-target=".submit-artwork-auction-modal">Đăng ký đấu giá</button>}
                  {nft.nftStatus === 'INITIAL' && <button onClick={() => this.props.handleClick(id, nft.nftId, idx)} type="button" className="btn btn-sm btn-outline-dark" data-toggle="modal" data-target=".transfer-artwork-modal" >Chuyển nhượng</button>}
                </div>
                {nft.nftStatus === 'READYFORAUC' && <span className="badge badge-info" >Sẵn sàng đấu giá</span>}
              </div>
            </div>
          </div>
        })}
