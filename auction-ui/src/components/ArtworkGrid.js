...
class ArtworkGrid extends Component {
   ...
   updateOnSocketMessage() {
      ...
      if (
         !this.state.isLoadingOpenAuctions &&
         !this.state.isLoadingYourArtwork
      ) {
      ...
      return (
         <div className='row mb-5'>
            ...
            {yourArtwork
               .sort(
                  (a, b) =>
                     new Date(b.item.timeStamp) - new Date(a.item.timeStamp)
               )
               .map((art, i) => (
                  <ArtworkCard
                     handleClick={(artworkIndex, nftID, nftIndex) =>
                        this.setState({
                           nftID: nftID,
                           nftIndex: nftIndex,
                           selectedNft: {
                              ...yourArtwork[artworkIndex]['nfts'][nftIndex],
                           },
                           selectedArtwork: yourArtwork[artworkIndex],
                           isBidding: false,
   ...
   renderOpenAuctions() {
      let { isLoadingOpenAuctions, openAuctions, isShowingAllOpenAuctions } =
         this.state
         ...
         <div className='row'>
            {openAuctions.length === 0 && (
               <div className='col-md-6 col-lg-4'>
                  <img
                     className='mb-4 mx-auto bg-primary'
                     src={emptyAuctions}
                     width='200'
                     alt='Empty Auction'
                  />
            ...
            {openAuctions.length > 0 &&
               openAuctions
                  .slice(0, isShowingAllOpenAuctions ? openAuctions.count : 3)
                  .map((auction, i) => (
                     <ArtworkCard
                        isAuction
                        handleClick={(artworkIndex, nftID) =>
                           this.setState({
                              nftID: nftID,
                              selectedArtwork: openAuctions[artworkIndex],
                              auctionID:
                                 openAuctions[artworkIndex]['auctionID'],
                              buyItNowPrice:
                                 openAuctions[artworkIndex]['buyItNowPrice'],
                              reservePrice:
                                 openAuctions[artworkIndex]['reservePrice'],
                              closeDate:
                                 openAuctions[artworkIndex]['closeDate'],
                              isBidding: true,
                              isViewingArtwork: true,
                           })
                        }
      ...
      return (
         <div className='container-fluid'>
               ...
               {this.state.newNotificationCount > 0 && (
                  <div className='col-md-6 mb-3 d-flex align-items-center justify-content-end'>
                     <button
                        ...
                        onClick={this.reloadAuctions}
                        data-toggle='tooltip'
                        title='Nhấn vào đây để tải phiên đấu giá mới'
                     >
                        Tải mới{' '}
            ...
            <div className='row mb-3 mt-3'>
               <div className='col-md-6'>
                  <h3 className='text-muted'>
                     <b>Phiên đấu giá đang diễn ra ({openAuctions.length})</b>
                  </h3>
               </div>
               <div className='col-md-6'>
                  {openAuctions.length > 3 && (
                     <a
                        ...
                        onClick={() =>
                           this.setState({
                              isShowingAllOpenAuctions:
                                 !this.state.isShowingAllOpenAuctions,
                           })
                        }
                     >
                        {isShowingAllOpenAuctions
            ...
            <div className='row mb-3 mt-5'>
               <div className='col-md-6'>
                  <h3 className='text-muted'>
                     <b>Tác phẩm của bạn ({yourArtwork.length})</b>
                  </h3>
               </div>
               <div className='col-md-6'>
                  <button
            ...
            <ArtworkDetail
               ...
               isVisible={this.state.isViewingArtwork}
               isAuction={isBidding}
               auctionID={this.state.auctionID}
               buyItNowPrice={this.state.buyItNowPrice}
            ...
            <TransferArtwork
               nftID={this.state.nftID}
               handleTransfer={() => this.getYourArtwork()}
               {...selectedArtwork}
            />
            {/* <div className='row'>
               <BlockchainExplorer
                  onSocketMessage={this.updateOnSocketMessage}
               />
            </div> */}
         </div>
      )
