...
class AuctionTableRow extends Component {
   ...
   componentDidMount() {
      if (this.props.status === 'OPEN' && this.state.interval.length === 0) {
         this.state.interval.push(
            setInterval(() => this.getHighestBidForAuction(), 1500)
         )
      }
      $(function () {
   ...
   getHighestBidForAuction() {
      this.auctions
         .getHighestBidForAuctionWithId(this.props.auctionID)
         .then(response => {
            this.setState({
   ...
   render() {
      ...
      return (
         <tr className='auction'>
            <td id='itemImageName' className='artwork'>
               <img
                  src={`${API_ENDPOINT}/images/${itemImageName}`}
                  width='100'
                  height='100'
                  alt='Artwork'
               />
            ...
            <td id='status'>
               <span className={`badge badge-${status}`}>
                  {AUCTION_STATUS.get(status)}
               </span>
            </td>
            <td id='reservePrice'>
               ${parseInt(reservePrice, 10).toLocaleString()}
            </td>
            <td id='buyItNowPrice'>
               ${parseInt(buyItNowPrice, 10).toLocaleString()}
            </td>
            <td id='requestDate'>
               {status === 'INIT' &&
                  moment(requestDate, REQUESTDATE_FORMAT).format('MM-DD-YYYY')}
               {status === 'OPEN' && (
                  <CountdownTimer
                     auctionId={auctionID}
                     endDate={closeDate}
                     handleCloseAuction={this.handleCloseAuction}
                  />
               )}
               {status === 'CLOSED' && '-'}
            </td>
            <td id='action'>
               {status === 'INIT' && (
                  <button
                     ...
                     data-toggle='modal'
                     data-target='.open-auction-modal'
                  >
                     Bắt đầu đấu giá
                  </button>
               )}
               {status === 'OPEN' && (
                  <div>
                     <strong>
                        $
                        {this.state.highestBidUsername
                           ? parseInt(
                                this.state.highestBidForAuction,
                                10
                             ).toLocaleString()
                           : '0'}
                     </strong>
                     ...
                     {/* <strong>${this.state.highestBidForAuction}</strong> */}
                  </div>
               )}
            </td>
         </tr>
