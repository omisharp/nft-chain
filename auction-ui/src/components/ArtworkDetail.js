...
class ArtworkDetail extends Component {
   ...
   loadNftID(nftID) {
      if (
         ...
         !Object.keys(this.state.artwork).length
      ) {
         ...
         this.dataLoader[1] = this.artwork
      ...
      return (
         <div className='container-fluid'>
            <div className='row'>
               <div
                  className={
                     'col-md-12 ' + (this.state.isAuctionClosed ? 'closed' : '')
                  }
               >
                  {!nft.aesKey && (
                     <div className='watermark watermark-full'></div>
                  )}
                  {!nft.aesKey && (
                     <img
                        className='img-stretch artwork-detail'
                        src={`${API_ENDPOINT}/images/${item.itemImageName}`}
                        alt='Artwork'
                     />
                  )}
                  {nft.aesKey && (
               ...
               <div className='col-md-12 mt-2 d-flex justify-content-between'>
                  {!isAuction && (
                     <h3>${parseInt(nft.price, 10).toLocaleString()}</h3>
                  )}
                  <h3>NFTID: {nftID}</h3>
               </div>
                        ...
                        <div className='collapse' id='collapseExample'>
                           <div className='list-group'>
                              {this.state.history.map((historyItem, i) => (
                                 <HistoryRow
                                    key={i}
                                    transactionId={historyItem.txID}
                                    timestamp={historyItem.timestamp}
                                    status={historyItem.nftStatus}
               ...
               {isAuction && (
                  <div className='col-md-12'>
                     <BiddingArea
                        ...
                        closeDate={closeDate}
                        closeAuction={this.handleCloseAuction}
                     />
                  </div>
               )}
            </div>
   ...
   render() {
      ...
      return (
         <div
            id='artworkDetailModal'
            className='modal fade art-detail-modal'
            tabIndex='-1'
            role='dialog'
            aria-labelledby='artDetail'
            aria-hidden='true'
         >
               ...
               <div className='modal-content'>
                  <div className='modal-header'>
                     <h5 className='modal-title'>
                        {item ? item.itemDetail : 'Artwork Detail'}
                     </h5>
                     <button
                        type='button'
                        className='close'
...
const HistoryRow = function (props) {
   return (
      <li className='list-group-item flex-column align-items-start'>
         <p className='mb-1'>
            Trạng thái của tác phẩm nghệ thuật được ghi lại là{' '}
            <span className='text-success'>{props.status}</span> trên
            blockchain,
            <br /> với chủ sở hữu là{' '}
            <span className='text-danger'>
               {props.ownerId} vào lúc {/*props.timestamp*/}{' '}
               {moment(
                  String(props.timestamp).replace(' +', '+').replace(' UTC', '')
