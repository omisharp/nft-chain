...
class CountdownTimer extends Component {
   ...
   countDownAuction() {
      ...
      let isCloseDateInPast = moment
         .utc(endDate, 'YYYY-MM-DD HH:mm:ss')
         .isBefore(moment.utc())
   ...
   getCoundownString(endDate) {
      ...
      if (timerObj.days) {
         timer +=
            String('00' + timerObj.days).slice(-pad.length) +
            (timerObj.days > 1 ? ' days ' : ' day ')
      }

      timer +=
         '' +
         String('00' + timerObj.hours).slice(-pad.length) +
         ':' +
